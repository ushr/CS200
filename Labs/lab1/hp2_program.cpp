...
void Program::GameLoop()
{
	...
	while (!isDone)
	{
		...
		cout << "Player " << turn << ", Enter an x coordinate : >> ";
		int x = GetValidInput(0, 2);




		cout << "Player" << turn << ", Enter an y coordinate : >>  ";
		int y = GetValidInput(0, 2);

		PlaceMarker(x, y, playerMarker[turn]);
		//gameBoard[x][y] = playerMarker[turn];
		SwapTurn();
		CheckForWinner();
		...
		if (CheckForWinner() == 0 || CheckForWinner() == 1)
		{
			isDone = true;
		}
...
void Program::DrawStats()
{
	...
	cout << endl << endl;
	cout << "Player 0 wins : " << playerWins[0] << "\t" << "\t" << "\t" << "Player 1 wins : " << playerWins[1] << endl << endl;
